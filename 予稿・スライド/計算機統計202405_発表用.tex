...
\node[above = of mouse, yshift = -20pt] (labmouse){マウス};
\node[above = of lemur, yshift = -20pt](lablemur){サル};
\node[above = of human, yshift = -20pt](labhuman){ヒト};
...
\node[draw, fill=gray!10, text height=1.25ex, text width=8.0ex, below=of mouse2, yshift = 2ex](mouse3){   };
\node[draw, right = of mouse3, text height=1.25ex, text width=8.0ex](lemur3){};
\node[draw, fill=gray!10, right = of lemur3, text height=1.25ex, text width=8.0ex, align=center](human3){};

\node[draw,  fill=gray!10, text height=1.25ex, text width=8.0ex, below=of mouse3, yshift = 2ex](mouse4){   };
...
\path[draw, RoyalBlue](labA2.north west)--(human2.north east);
\path[draw, RoyalBlue](labA2.north west)--(labC.south west);
\path[draw, RoyalBlue](human2.north east)--(human4.south east);
\path[draw, RoyalBlue](labC.south west)--(human4.south east)node[midway, below](int){data integration};
\end{tikzpicture}
...
\begin{figure}
\begin{tabular}{c|c}
multi-dimensional array & tidy format\\
\hline
...
\begin{align*}
y_{n}  \approx\ &\sum_{l=1}^L\prod_{d=1}^D v_{dl}^{x_{nd}} =\frac{1}{L}\sum_{l=1}^L   L \underbrace{\exp\left( \sum_{d=1}^D x_{nd}  \cdot \log v_{dl}\right) }_{\mbox{$f_{nl}$}}
%&= \frac{1}{L}\sum_{l=1}^L L f_{nl} \quad \mbox{\structure{(mean of the log-linear predictor)}}
\end{align*}
\begin{figure}
\includegraphics[height=0.5\textheight]{img/ensemble}
\caption{x軸: 推定値, y軸: 実測値, 色付きの点: $L f_{nl}$, 黒い点: $\sum_l f_{nl}$}
\end{figure}
}
\frame{
\frametitle{モデル（準備）}

式1$\left( y_{n} \approx \sum_{l} \prod_{d=1}^D v_{dl}^{x_{nd}}\right)$ より
\begin{align}
...
\end{tabular}
\caption{左: 潜在変数の事前分布. 右: 観測されるデータのモデル. }
\end{figure}

観測されるデータのモデルは0の一点で確率を持つ

（cf. zero-inflated モデル）

\vspace{\baselineskip}

\structure{Note:} モデルの密度が0になることは対数尤度に$-\infty$の罰則をつけることと同じであるため, 分布の台が最も大きい影響を持つと考えた.
}
\frame{
\frametitle{モデル}
