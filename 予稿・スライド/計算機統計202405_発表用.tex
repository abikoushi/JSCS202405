...
\begin{itemize}
\item variational E-step: draw $\tilde{z}_n$ for all $n$s using Eq.\ref{qz_rect}--\ref{qz_binary}
\item variational M-step: update $q(v_{dl})$ using Eq.\ref{qv} and update $q(\lambda)$ using Eq.\ref{qlam}
\end{itemize}
}
\frame{
